...
<<echo=FALSE,fig.height=5,fig.width=5,out.width='0.8\\linewidth'>>=
par(mar=c(5,5,0,0))
plot(x1,y1)
abline(m1)
hist(m1$residuals,breaks=30,main=NA)
...
\centering
<<echo=FALSE,fig.height=5,fig.width=6,out.width='0.8\\linewidth'>>=
...
library(ggplot2)
ggplot(full.df,aes(x,y)) + geom_line(size=1,color='black') + geom_line(data=full.df2,aes(x,y),color='red',size=1) + theme_bw(base_size=20) + plotheme + ylab(NULL) + xlab(NULL) + coord_cartesian(x=c(10,50)) + annotate('text',x=40,y=0.35,label="P(E(Y_h))",colour='red') + annotate('text',x=40,y=0.30,label="P(Y_h_novo)",colour='black')
@
...
Eleft <- E_yh-error
Eright <- E_yh+error
curve(dnorm(x,10,2),xlim=c(0,20),col='blue',lwd=2,xlab="E(Y_h)",ylab=NA)
...
\column{0.5\linewidth}
<<histoiaf,echo=FALSE,fig.height=5,fig.width=5,out.width='1\\linewidth'>>=
df <- read.csv('C:/Users/thiago/OneDrive/UNESP/Pos_graduacao/Eco/2015/Estatistica_2015/Aulas/Aula_4_Regressao_simples/ndvi_iaf.csv')
names(df) <- c('x','y')
df$y <- log(df$y)
hist(df$y,breaks=20,col='gray50',main=NA)
@


\column{0.5\linewidth}
<<scatteriaf,echo=FALSE,fig.height=5,fig.width=5,out.width='1\\linewidth'>>=
plot(y~x,data=df)
miaf <- lm(y~x,data=df)
abline(miaf)
@
...
<<echo=F,plotsqt,fig.height=5,fig.width=5,out.width='1.1\\linewidth'>>=
# Plot de SQ_{tot}
par(mar=c(3,3,0,0))
plot(x,y,pch=19)
abline(h=ybar,lty=2,lwd=2)
segments(x0=x,y0=y,x1=x,y1=ybar,col='black',lwd=2)
text(x=20,y=150,label=expression(paste("SQtot = ", sum((Y[i] - bar(Y))^2)," = 41202")),cex=1.5,col='black')
...
<<echo=F,plotsqE,fig.height=5,fig.width=5,out.width='1.1\\linewidth'>>=
# Plot de SQ_{res}
par(mar=c(3,3,0,0))
plot(x,y,pch=19)
abline(m,lty=1,lwd=2)
segments(x0=x,y0=y,x1=x,y1=ypred,col='blue',lwd=2)
text(x=20,y=150,label=expression(paste("SQres = ",sum((Y[i] - hat(Y[i]))^2)," = 6662")),cex=1.5,col='blue')
legend("bottomright",legend = expression(hat(Y[i])),lty=1,lwd=2,cex=1.5,bty="n")
@
...
<<echo=F,plotsqr,fig.height=5,fig.width=5,out.width='1.1\\linewidth'>>=
# Plot de SQ_{reg}
par(mar=c(3,3,0,0))
plot(x,y,pch=19)
abline(m,lty=1,lwd=2)
...
text(x=20,y=150,label=expression(paste("SQreg = ",sum((hat(Y[i]) - bar(Y))^2)," = 34539")),cex=1.5,col='red')
...
<<c1,fig.height=5,fig.width=5,out.width='0.6\\linewidth',echo=FALSE>>=
set.seed(6267)
x1 <- runif(20,0,5)
set.seed(6223)
x2 <- runif(20,10,20)
